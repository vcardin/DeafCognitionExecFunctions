...
%cd=marsbar_dir;
mainfolder_Data='/Users/deafneuralplasticitylab/Documents/MATLAB/1-First Level analysis/Lev1st_WM_buttonpressbyhand/';%changes on the base of the task
...
subjs={'002'; '004'; '006';'007';'008';'011'; '013';'014'; '015'; '016'; '017'; '018';'021';'023';'028';'031';'032';'041'; '042';'101'; '103'; '104';  '106'; '107';'108'; '110';'111';'114'; '115'; '116'; '118';'119'; '122';'124';'127'; '129'; '130';  '131'; '132'; '133';'134';'135'; '136'}; %WM
%subjs={'114'};
...
for n=1:size(subjs,1)
    ...
    % select the STC ROI
    r = dir(fullfile(roi_dir,'Right_Te1.0_roi.mat'));
    LTe10_name = [r.name];
    roinames  = strcat(roi_dir,'/' ,LTe10_name);
...
%   wm_file  = strcat(subjroot ,'/',wm_name);
    ...
    %regionname = char(region_name(Y)); %note the regionname   
     ...
     data_file= fullfile (roi_dir,'WorkingMemory_Right_Te1.0_roi.mat');
...
marsbar_dir='/Users/b.manini/Documents/MATLAB/spm12/toolbox/marsbar-0.44';
%cd=marsbar_dir;
mainfolder_Data='/Users/deafneuralplasticitylab/Documents/MATLAB/Velia_ROI_contrast_analysis/Lev1st_WM_buttonpressbyhand/';%changes on the base of the task
mainfolder_ROI='/Users/deafneuralplasticitylab/Documents/MATLAB/Data/ROI_freesurf/';
% Start marsbar to make sure spm_get works
marsbar('on')
%all the subjs (select on the base of the task)
%This are all the deaf subject who have TS data
%subjs={'101';'104'; '106'; '107';'108'; '110';'111'; '115'; '116'; '118';'119'; '122';'124';'127'; '129'; '130';  '131'; '132'; '133';'134'; '136'};
%subjs={ '002'; '004'; '006';'007';'008';'011'; '013';'014'; '015'; '016'; '017'; '018';'021';'023';'028';'031';'032';'041'; '042';'101'; '104';  '106'; '107';'108'; '110';'111'; '114';'115'; '116'; '118';'119'; '122';'124';'127'; '129'; '130';  '131'; '132'; '133';'134'; '135';'136'}; %WM
subjs={'103'};
% MarsBaR version check
if isempty(which('marsbar'))
  error('Need MarsBaR on the path');
...
for n=1:size(subjs,1)
    ...
    % select the STC ROI
    r = dir(fullfile(roi_dir,'PTwithoutSTC_roi.mat'));
    stc_R_name = [r.name];
    roinames  = strcat(roi_dir,'/' ,stc_R_name);
    %roi_array{1} = maroi(stc_R_name); %right STC
    
    %load con file
    P= dir(fullfile(subjroot ,'con_0009.nii')); % con= 0 1 0 0 working memory task % get images from contrast directory where they are stored
    V = spm_vol(P);
%     wm_file  = strcat(subjroot ,'/',wm_name);
    D = strvcat(V.name); %load images into format for Marsbar  
    D2= strcat(subjroot,'/' , D);
    ...
    regionname = char(region_name(Ywm)); %note the regionname   
      ...
      %load con file control task
    K= dir(fullfile(subjroot ,'con_0010.nii')); % con= 1 0 0 0 working memory task % get images from contrast directory where they are stored
    J = spm_vol(K);
%   wm_file  = strcat(subjroot ,'/',wm_name);
    H = strcat(J.name); %load images into format for Marsbar  
    H2= strcat(subjroot,'/' , H);
    ...
    %regionname = char(region_name(Y)); %note the regionname   
     ...
     data_file= fullfile (roi_dir,'WM_resp_PTwithoutSTC_roi.mat');
     save(data_file, 'datamean_wm', 'datamean_con','regionname'); 
