...
mainfolder_Data='/Users/velia/Documents/DeafCognition/FirstLevel/Lev1st_TowerofLondon/';

%Replace with the directory where your ROIs are kept (the script will automatically loop into each participant folder)
mainfolder_ROI=['/Users/velia/Library/CloudStorage/OneDrive-UniversityCollegeLondon/1_Research/DeafCognition/Analysis/ROI/2023_DMN_Task/ROIsFoxPNAS']; 
...
%      data_file= fullfile (roi_dir,'/WorkingMemory_ preSMA_roi.mat');
					...
					all_data(n,:,r)=[ datamean_tol, datamean_con, str2num(char([subjs(n,:)]))];
     
     clear subjroot roi_dir  L_HG_name  P V D D2 J K H H2 datamean_tol datavar_tol o_tol region_name datamean_con datavar_con o_con Ytol Wcon %data_file
...
header{c}=strcat('ToL_',roi_list(l).name);
c=c+1;
